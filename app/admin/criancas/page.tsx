...
import { addKid, addKidsBulk } from "./actions";
...
export default async function AdminKids() {
  const { rows } = await db.query<KidRow>(
    "SELECT id, name, confirmed_at, declined_at, created_at FROM kids_list ORDER BY name ASC"
  );
    ...
    <main className="mx-auto flex w-full max-w-5xl flex-1 flex-col px-5 py-10 sm:px-6 sm:py-12">
      ...
      <section className="mt-6 grid gap-4 sm:mt-8 sm:grid-cols-2 lg:grid-cols-4">
        <div className="rounded-lg border border-accent/20 bg-white p-5 sm:p-6">
          <p className="font-sans text-sm text-muted">Crianças</p>
          <p className="mt-1 font-serif text-3xl text-accent sm:text-4xl">
            {total}
          </p>
        </div>
        <div className="rounded-lg border border-accent/20 bg-white p-5 sm:p-6">
          <p className="font-sans text-sm text-muted">Confirmadas</p>
      ...
      <section className="mt-6 grid gap-4 sm:mt-8 lg:grid-cols-2">
        <div className="rounded-lg border border-accent/20 bg-white p-5 sm:p-6">
          ...
          <form action={addKid} className="mt-4 flex gap-2">
            <input
              name="name"
              type="text"
              required
        ...
        <div className="rounded-lg border border-accent/20 bg-white p-5 sm:p-6">
          ...
          <form action={addKidsBulk} className="mt-4 space-y-3">
            <textarea
              name="names"
              rows={4}
              required
